...
/**
 * Gets the secondary chat ID from environment variables, Supabase, or LocalStorage.
 */
export const getSecondaryChatId = async (): Promise<string> => {
  ...
  const localId = localStorage.getItem("telegram_secondary_chat_id")
  if (localId) return localId

  // 3. Check Supabase (if needed)
  try {
    const { data } = await supabase
      .from("settings")
      .select("value")
      .eq("key", "telegram_secondary_chat_id")
      .maybeSingle()
    if (data?.value) {
      localStorage.setItem("telegram_secondary_chat_id", data.value)
      return data.value
    }
  } catch (e) {
    console.warn("⚠️ Could not fetch secondary chat ID from Supabase", e)
  }
...
/**
 * Saves the secondary chat ID to LocalStorage and Supabase.
 */
export const setSecondaryChatId = async (id: string): Promise<void> => {
  ...
  try {
    ...
    await supabase
      .from("settings")
      .upsert({ key: "telegram_secondary_chat_id", value: id }, { onConflict: "key" })
  } catch (e) {
    console.error("❌ Failed to save secondary Chat ID to Supabase", e)
  }
}
...
export const sendTelegramOrderNotification = async (order: Order | any) => {
  ...
  const { shippingDetails, items, total, orderNumber, id } = order
...
🆔 <b>رقم الطلب:</b> <code>${orderNumber || id.slice(0, 8).toUpperCase()}</code>
